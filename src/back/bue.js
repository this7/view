var bue = (function() {
    ...
    //textParser-开始
    //======================================================================================
    var textParser.parse = function(text) {
        const tagRE = /\{?\{\{(.+?)\}\}\}?/g;
    ...
    directive.parse = function(argument) {
        let dirs = [];
        if (s.indexOf(':') !== -1) {
    ...
    expParser.compileGetter = function(path) {
        path = path.split('.');
        let boby = 'if (o !=null';
        let pathString = 'o';
        let key;
     ...
     */
    util.define(objectAgumentations, '$add', function(key, val) {
    ...
    util.define(objectAgumentations, '$delete', function(key) {
        if (!this.hasOwnProperty(key)) return;
        delete this[key];
    });
    ...
    Bue.prototype = {
        ...
        _init: function(options) {
            ...
            util.extend(this.$options, this.constructor.options);
            if (this.$parent) {
        ...
        _initElement: function(el) {
            if (typeof el === 'string') {
                ...
                if (!el) {
                    util.warn(`Cannot find element: ${selector}`);
                }
            } else {
                this.$el = el;
            }
        ...
        initProp: function(path, val, dynamic) {
            if (!dynamic) {
                ...
                // 动态prop
                this.$data[path] = expression.compileGetter(val)(this.$parent.$data);
            }
        },
        /**
         * 注册事件及其回调函数到实例上
